...
// Group by file
const fileGroups = routeUpdates.reduce(
    (acc, update) => {
        if (!acc[update.file]) acc[update.file] = []
        acc[update.file].push(update)
        return acc
    },
    {} as Record<string, typeof routeUpdates>
)
...
for (const [fileName, updates] of Object.entries(fileGroups)) {
    ...
    for (const update of updates) {
        ...
        const endpointEscaped = update.endpoint.replace(/\//g, '\\/')

        // Pattern: find the endpoint, then find description line, then insert operationId before next field
